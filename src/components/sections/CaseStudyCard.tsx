...
export function CaseStudyCard({
  ...
  return (
    <div className="flex w-full flex-col overflow-hidden lg:h-180 lg:flex-row">
      <div className="flex flex-col gap-10 overflow-y-auto bg-white p-8 sm:p-12.5">
        {slide.logoUrl && (
        ...
        <div className="flex w-140 max-w-full flex-col gap-6">
          <h3 className="max-w-102.5 text-2xl font-medium text-black sm:text-[35px]">
            {slide.title}
          </h3>
          {slide.tags.length > 0 && (
            <div className="flex flex-wrap items-center gap-3 text-sm font-medium tracking-[-0.3px] text-black sm:gap-3.75 sm:text-base">
          ...
          <div className="grid grid-cols-1 gap-x-16 gap-y-5 rounded-[15px] bg-linear-to-r from-[#cde1ff]/50 to-[#d9daff]/50 px-5 py-3 font-medium sm:grid-cols-2">
            <div>
              <p className="text-sm tracking-[-0.27px] text-black/40">
                Services
              </p>
      ...
      {slide.mainImageUrl && (
        <div className="relative h-100 w-full lg:h-full lg:w-auto lg:flex-1">
          <Image
            src={slide.mainImageUrl}
            alt={slide.title}
            fill
            priority={priority}
            sizes="(min-width: 1024px) calc(100vw - 700px), 100vw"
